...
strFrame+='					   onclick="parent.meizzPrevM()" title="Prev 1 mon" ><b >&lt;</b></td>';
...
strFrame+='					   title="click to select year"><span  id=meizzYearHead></span></td>';
...
strFrame+='					   title="click to select month"><span id=meizzMonthHead ></span></td>';
strFrame+='				   <td width=16 bgcolor=#63A3E9 align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='					   onclick="parent.meizzNextM()" title="Next 1 mon" ><b >&gt;</b></td>';
strFrame+='			   </tr>';
...
strFrame+='<td style="font-size:12px;color:#FFFFFF" >&nbsp;Mo</td><td style="font-size:12px;color:#FFFFFF">&nbsp;Tu</td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" >&nbsp;We</td><td style="font-size:12px;color:#FFFFFF" >&nbsp;Th</td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" >&nbsp;Fr</td><td style="font-size:12px;color:#FFFFFF" >&nbsp;Sa</td></tr>';
strFrame+='</table></td></tr>';
...
strFrame+='         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="set date empty">emp</span>&nbsp;&nbsp;<span onclick="parent.meizzToday()" style="font-size:12px;cursor: hand"';
strFrame+='         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="current date">cur</span>&nbsp;&nbsp;<span style="cursor:hand" id=evaAllOK onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'"  onclick="parent.closeLayer()" title="close">close&nbsp;</span></td></tr>';
strFrame+='    </table></td></tr><tr ><td >';
strFrame+='        <table border=0 cellspacing=1 cellpadding=0 width=100%  bgcolor=#FFFFFF height=22 >';
strFrame+='          <tr bgcolor="#63A3E9"><td id=bUseTimeLayer width=30  style="cursor:hand" title="click to enable/disable time"';
...
strFrame+=' title="click to select time" align=center width=42>' ;
...
document.writeln('<iframe id="endDateLayer" frameborder=0 width=162 height=220 style="position: absolute;  z-index: 9998; display: none"></iframe>');
...
{
	return getDocumentAll().endDateLayer;
}

//主调函数
function setday(tt, obj)
{
	if (arguments.length > 2){alert("Sorry! Too much arguments!");return;}
	if (arguments.length == 0){alert("Sorry! No any arguments!");return;}
	var dads = getDateLayer().style;
	var th = tt;
...
function meizzWriteHead(yy,mm,ss)
{
	...
	//插入当前小时、分
	odatelayer.meizzHourHead.innerText=bUseTime?(meizzTheHour+" h"):""; 
	odatelayer.meizzMinuteHead.innerText=bUseTime?(meizzTheMinute+" m"):"";
	odatelayer.meizzSecondHead.innerText=bUseTime?(meizzTheSecond+" s"):"";
}

function tmpSelectYearInnerHTML(strYear) //年份的下拉框
{
	if (strYear.match(/\D/)!=null){alert("The year is not a number!");return;}
	var m = (strYear) ? strYear : new Date().getFullYear();
	if (m < 1000 || m > 9999) {alert("The year is not between 1000 and 9999!");return;}
	var n = m - 50;
	if (n < 1000) n = 1000;
	if (n + 101 > 9999) n = 9974;
	var s = "&nbsp;<select name=tmpSelectYear style='font-size: 12px' "
	s += "onblur='getDocumentAll().tmpSelectYearLayer.style.display=\"none\"' "
	s += "onchange='getDocumentAll().tmpSelectYearLayer.style.display=\"none\";"
...
{
	if (strMonth.match(/\D/)!=null){alert("The month is not a number!");return;}
	var m = (strMonth) ? strMonth : new Date().getMonth() + 1;
	var s = "&nbsp;&nbsp;&nbsp;<select name=tmpSelectMonth style='font-size: 12px' "
	s += "onblur='getDocumentAll().tmpSelectMonthLayer.style.display=\"none\"' "
	s += "onchange='getDocumentAll().tmpSelectMonthLayer.style.display=\"none\";"
	s += "parent.meizzTheMonth = this.value; parent.meizzSetDay(parent.meizzTheYear,parent.meizzTheMonth)'>\r\n";
	var selectInnerHTML = s;
	for (var i = 1; i < 13; i++)
	{
		if (i == m) { selectInnerHTML += "<option value='"+i+"' selected>" + i + " mon" + "</option>\r\n"; }
		else { selectInnerHTML += "<option value='"+i+"'>" + i + " mon" + "</option>\r\n"; }
	}
	selectInnerHTML += "</select>";
...
function tmpSelectHourInnerHTML(strHour) //小时的下拉框
{
	...
	if (strHour.match(/\D/)!=null){alert("The hour is not a number!");return;}
	var m = (strHour) ? strHour : new Date().getHours();
	var s = "<select name=tmpSelectHour style='font-size: 12px' "
	s += "onblur='getDocumentAll().tmpSelectHourLayer.style.display=\"none\"' "
	s += "onchange='getDocumentAll().tmpSelectHourLayer.style.display=\"none\";"
...
function tmpSelectMinuteInnerHTML(strMinute) //分钟的下拉框
{
	...
	if (strMinute.match(/\D/)!=null){alert("The minute is not a number!");return;}
	var m = (strMinute) ? strMinute : new Date().getMinutes();
	var s = "<select name=tmpSelectMinute style='font-size: 12px' "
	s += "onblur='getDocumentAll().tmpSelectMinuteLayer.style.display=\"none\"' "
	s += "onchange='getDocumentAll().tmpSelectMinuteLayer.style.display=\"none\";"
...
function tmpSelectSecondInnerHTML(strSecond) //秒的下拉框
{
	...
	if (strSecond.match(/\D/)!=null){alert("The second is not a number!");return;}
	var m = (strSecond) ? strSecond : new Date().getMinutes();
	var s = "<select name=tmpSelectSecond style='font-size: 12px' "
	s += "onblur='getDocumentAll().tmpSelectSecondLayer.style.display=\"none\"' "
	s += "onchange='getDocumentAll().tmpSelectSecondLayer.style.display=\"none\";"
...
{
	getDocumentAll().endDateLayer.style.display="";
}
...
function GetMonthCount(year,month)	//闰年Tu月为29天
{
...
function meizzPrevY()	//往前翻 Year
{
	...
	else{alert("The year is not between 1000 and 9999!");}
...
function meizzSetDay(yy,mm)	//主要的写程序**********
{
	...
	var day1 = 1,day2=1,firstday = new Date(yy,mm-1,1).getDay();	//某月第Mo天的星期几
	...
	for (i = 0; i < 37; i++)
	{
		var da = eval("odatelayer.meizzDay"+i)	//书写新的Mo个月的日期星期排列
	...
	if (meizzWDay[i]!="")
	{
		...
		{
			da.innerHTML="<b><font color=#BCBABC>" + meizzWDay[i] + "</font></b>";
			da.title=(mm==12?1:mm+1) +"mon" + meizzWDay[i] + "day";
			da.onclick=Function("meizzDayClick(this.innerText,1)");
			if(!outDate)
...

		{
			...
			da.title=mm +"mon" + meizzWDay[i] + "day";
...
function meizzDayClick(n,ex)	//点击显示框选取日期，主输入函数*************
{
	...
	if (outObject)
	{
		...
		if (bUseTime)
		{
			...
			{
				setday(outObject, null);
			}
		}
	}
	else {closeLayer(); alert("The component is not exist!");}
}
...
function evaSetTime()		//设置用户选择的小时、分钟
{
	...
	odatelayer.meizzHourHead.innerText=meizzTheHour+" h";
	odatelayer.meizzMinuteHead.innerText=meizzTheMinute+" m";
	odatelayer.meizzSecondHead.innerText=meizzTheSecond+" s";
...
{
	odatelayer.meizzHourHead.innerText=new Date().getHours()+" h";
	odatelayer.meizzMinuteHead.innerText=new Date().getMinutes()+" m";
	odatelayer.meizzSecondHead.innerText=new Date().getSeconds()+" s";
