...
<table width="99%" border="0" cellspacing="0" cellpadding="0"
	...
	<tr>
		<td class="table1">
			<img src="<c:url value="/resources/images/arrow.gif"/>" width="20" 
				height="18" hspace="4" align="absmiddle">
	...
	<tr>
		...
		<form name="orderQuerier" id="orderQuerier"
			action="<c:url value="/produceOrders.ord"/>" method="post">
			...
			<tr>
				<td>
					<table class="table9" width="100%" border="1" cellpadding="0" cellspacing="0"
						...
						<tr>
							<td align="left" width="25%" class="table8">&nbsp;<spring:message
				...
				<td>
					<input type="submit" width="74" height="22" align="absmiddle" class="button"
						value="<spring:message code="operation.query"/>"/>
				</td>
			</tr>
		</table>
		</form>
	...
	<tr>
		...
		<form name="batchCommandForm" id="batchCommandForm"
			action="<c:url value="/produceBatchCommand.ord"/>" method="post"
			onsubmit="return checkSelected()">
		<table width="99%" height="40" border="0" align="center"
			cellpadding="0" cellspacing="0">
			<tr class="fy">
				<td width="50%" align="left">
					&nbsp;<label><input name="selectAll" type="checkbox" value="on" onclick="javascript:checkAll(this.form, this.checked);"/><spring:message code="operation.selectAll"/></label>
					&nbsp;<input name="method" type="submit" class="button" align="middle" value="<spring:message code="operation.batchPrint"/>" onclick="setOperation(0)"/>
					...
					&nbsp;<input type="button" class="button" onclick="javascript:window.location.href='<c:url value="/produceExportOrders.ord"/>'" value="<spring:message code="operation.export"/>"/>
					&nbsp;<input name="method" type="submit" class="button" align="middle" value="<spring:message code="operation.packprints"/>" onclick="setOperation(4)"/>
				</td>
				<td width="25%" align="left" valign="middle">
					<spring:message	code="nav.records"/>:&nbsp;
					<c:out value="${orderQuerier.recordCount}"/>&nbsp;
					<spring:message code="nav.current"/>&nbsp;
					<c:out value="${orderQuerier.pageNo + 1}"/>/
					<c:out value="${orderQuerier.pageCount}"/>&nbsp;
					<spring:message	code="nav.page"/>
				</td>
				<td width="25%" align="center">
					<a href="<c:url value="/produceOrders.ord"><c:param name="pageNo" value="0"/></c:url>">
						<b><spring:message code="nav.first"/></b>
					</a>
					<a href="<c:url value="/produceOrders.ord"><c:param name="pageNo" value="${((orderQuerier.pageNo - 1) > 0) ? (orderQuerier.pageNo - 1) : 0}"/></c:url>">
						<b><spring:message code="nav.prev"/></b>
					</a>
					<a href="<c:url value="/produceOrders.ord"><c:param name="pageNo" value="${((orderQuerier.pageNo + 1) < (orderQuerier.pageCount - 1)) ? (orderQuerier.pageNo + 1) : (orderQuerier.pageCount - 1)}"/></c:url>">
						<b><spring:message code="nav.next"/></b>
					</a>
					<a href="<c:url value="/produceOrders.ord"><c:param name="pageNo" value="${orderQuerier.pageCount - 1}"/></c:url>">
		...
		<table width="99%" border="1" align="center" cellpadding="0"
			...
			<tr class=table7>
				<td width="3%">&nbsp;</td>
				<td width="6%"><spring:message code="order.printtimes" /></td>
				<td width="8%"><spring:message code="order.id" /></td>
				<td width="15%"><spring:message code="order.clientname" /></td>
				<td width="11%"><spring:message code="order.ordertype" /></td>
			...
			<c:forEach var="order" items="${orderQuerier.listOrder}"
				...
				<td>
					<c:forEach var="variable"
						items="${order.processInstance.tokenVariables}"
						varStatus="variableStatus">
						<c:if test="${fn:toLowerCase(variable.name) == 'printtimes'}">
							<c:out value="${variable.stringvalue}"/>&nbsp;
						</c:if>
					</c:forEach>&nbsp;
				</td>
				<td>
				...
				<td>
					<c:choose>
						<c:when
							test="${order.processInstance.token < 3}">
							--
						</c:when>
						<c:otherwise>
							<c:forEach var="node"
								items="${order.processInstance.nodeInstances}"
				...
				<td>
					<a href="<c:url value="/produceViewOrder.ord"><c:param name="orderId" value="${order.id}"/></c:url>">
					...
					&nbsp;<a href="<c:url value="/producePrintOrder.ord"><c:param name="orderId" value="${order.id}"/></c:url>">
						<b><spring:message code="operation.print"/></b>
						</a>
					&nbsp;<a href="<c:url value="/producePackingPrint.ord"><c:param name="orderId" value="${order.id}"/></c:url>">
						<b><spring:message code="operation.packprint"/></b>
						</a>
				</td>
				</tr>
			</c:forEach>
		...
		<table width="99%" height="40" border="0" align="center"
			...
			<tr class="fy">
				<td width="50%" align="left">
					...
					&nbsp;<input name="method" type="submit" class="button" align="middle" value="<spring:message code="operation.batchCommit"/>" onclick="setOperation(2)"/>
					&nbsp;<input type="button" class="button" onclick="javascript:window.location.href='<c:url value="/produceExportOrders.ord"/>'" value="<spring:message code="operation.export"/>"/>
				    &nbsp;<input type="button" class="button" onclick="javascript:window.location.href='<c:url value="/producePackingPrints.ord"/>'" value="<spring:message code="operation.packprints"/>"/>
				</td>
				<td width="25%" align="left" valign="middle">
					<spring:message	code="nav.records"/>:&nbsp;
					<c:out value="${orderQuerier.recordCount}"/>&nbsp;
