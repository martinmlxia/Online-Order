...
<table width="99%" border="0" cellspacing="0" cellpadding="0"
	...
	<tr>
		<td>
			<form name="orderQuerier" id="orderQuerier"
				action="<c:url value="/salesOrders.ord"/>" method="post">
	...
	<tr>
		...
		<form name="batchCommandForm" id="batchCommandForm"
			action="<c:url value="/salesBatchCommand.ord"/>" method="post"
		...
		<table width="99%" height="40" border="0" align="center"
			...
			<tr class="fy">
				<td width="60%" align="left">
					...
					&nbsp;<input type="button" class="button" onclick="javascript:window.location.href='<c:url value="/salesExportOrders.ord"/>'" 
				...
				<td width="20%" align="center">
					<a href="<c:url value="/salesOrders.ord"><c:param name="pageNo" value="0"/></c:url>">
						<b><spring:message code="nav.first"/></b>
					</a>
					<a href="<c:url value="/salesOrders.ord"><c:param name="pageNo" value="${((orderQuerier.pageNo - 1) > 0) ? (orderQuerier.pageNo - 1) : 0}"/></c:url>">
						<b><spring:message code="nav.prev"/></b>
					</a>
					<a href="<c:url value="/salesOrders.ord"><c:param name="pageNo" value="${((orderQuerier.pageNo + 1) < (orderQuerier.pageCount - 1)) ? (orderQuerier.pageNo + 1) : (orderQuerier.pageCount - 1)}"/></c:url>">
						<b><spring:message code="nav.next"/></b>
					</a>
					<a href="<c:url value="/salesOrders.ord"><c:param name="pageNo" value="${orderQuerier.pageCount - 1}"/></c:url>">
		...
		<table width="99%" border="1" align="center" cellpadding="0"
			...
			<tr class="table7">
				...
				<td width="5%"><spring:message code="lens.price"/></td>
				<td width="10%"><spring:message code="order.invoiceid"/></td>
				<td width="15%"><spring:message code="order.operation"/></td>
			</tr>
			<c:forEach var="order" items="${orderQuerier.listOrder}" varStatus="itemStatus">
				...
				<td><c:out value="${order.user.user_tag}"/></td>
				<td>
				...
				<td>
					<a href="<c:url value="/salesViewOrder.ord"><c:param name="orderId" value="${order.id}"/></c:url>">
					<b><spring:message code="operation.view"/></b>
					</a>
					<c:if test="${order.processInstance.token < 2}">
					&nbsp;<a href="<c:url value="/salesModifyFrameLensOrder.ord"><c:param name="orderId" value="${order.id}"/></c:url>">
