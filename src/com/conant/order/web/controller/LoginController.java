...
public class LoginController extends SimpleFormController
{
			...
			throws ProcessException, Exception
	{
		...
		try
		{
			...
			session.setAttribute("user_name", loginInfo.getUserName()); // 用户帐号(user_tag)
			session.setAttribute("loginId", loginId); // 随机数

			session.setAttribute("userName", userData.getUser_name()); // 用户姓名(user_name)
			session.setAttribute("user_id", userData.getUser_id()); // 用户id(user_id)
